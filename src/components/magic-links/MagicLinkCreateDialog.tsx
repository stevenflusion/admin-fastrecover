...
function MagicLinkCreateDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("[MAGIC-CREATE] payload:", JSON.stringify(input, null, 2))
      const link = await createMagicLink(input)
      setCreatedLink(link)
      toast.success("MagicLink creado exitosamente")
      onCreated?.(link)
    } catch (err) {
      console.error("[MAGIC-CREATE] createMagicLink failed:", err)
