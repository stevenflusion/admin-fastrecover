...
export default function LeadsPage() {
  ...
  React.useEffect(() => {
    ...
    getLeadsWithQuotes(filters)
      ...
      .catch((err) => {
        console.error("[LEADS-PAGE] getLeadsWithQuotes failed:", err)
        if (!cancelled) {
          setLoading(false)
        }
      })
  ...
  const refreshLead = (updated: Lead) => {
    setLeads((prev) =>
      prev.map((lead) => (lead.id_lead === updated.id_lead ? updated : lead))
    )
    setSelectedLead((prev) =>
      prev?.id_lead === updated.id_lead ? updated : prev
    )
  }
